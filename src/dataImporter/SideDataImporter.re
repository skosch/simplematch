...
let make =
  ...
  shouldUpdate: ({oldSelf, newSelf}) => {
    ...
    let newIncludeSelectees =
      oldSelf.state.prevIncludeSelectees^ !== includeSelectees;
    let newIgnoredRowIndices =
      oldSelf.state.prevIgnoredRowIndices^ !== ignoredRowIndices;
  ...
  render: (_) => {
    ...
    let rowHeaders = index =>
      "<span class=\""
      ++ (Set.has(ignoredRowIndices, index) ? "ignored-row" : "")
      ++ "\" title=\""
      ++ (
        Set.has(ignoredRowIndices, index) ?
          "Row has errors and will be ignored" : ""
      )
      ++ "\">"
    ...
    let changeHandler = (changes, source) =>
      if (source !== "loadData") {
        let newRawData = rawData |. applyChanges(changes, maxCols);
